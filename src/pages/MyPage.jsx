    ...
    <div className="py-12 md:py-16 max-w-5xl mx-auto px-4 sm:px-6">
      ...
      <div className="mb-10">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[#144A42] flex items-center gap-2">
        ...
        {pet ? (
          <div className="bg-white p-6 border border-[#E8E1D2] shadow-xs flex flex-col sm:flex-row items-center gap-6">
            <div className="w-24 h-24 overflow-hidden bg-gray-100 shrink-0 border-2 border-[#144A42]">
              <img 
            ...
            <div className="flex-1 space-y-2 text-xs w-full text-center sm:text-left">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center justify-center sm:justify-start gap-2">
                  ...
                  <span className="bg-[#EAF5F2] text-[#144A42] font-bold px-2 py-0.5 text-[11px]">
                    {pet.status}
                  </span>
                </div>
                <span className="text-[11px] font-mono text-gray-500 bg-gray-50 px-2.5 py-1 border border-gray-200 inline-block">
                  등록번호: {pet.regNumber}
                </span>
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 text-[#5A6862]">
                <div className="bg-[#FAF9F6] p-2.5 border border-gray-100">
                  <span className="text-gray-400 block text-[10px]">품종</span>
                  <span className="font-semibold">{pet.breed}</span>
                </div>
                <div className="bg-[#FAF9F6] p-2.5 border border-gray-100">
                  <span className="text-gray-400 block text-[10px]">성별 / 중성화</span>
                  <span className="font-semibold">{pet.gender} ({pet.neutered})</span>
                </div>
                <div className="bg-[#FAF9F6] p-2.5 border border-gray-100">
                  <span className="text-gray-400 block text-[10px]">생년월일</span>
                  <span className="font-semibold">{pet.birth}</span>
                </div>
                <div className="bg-[#FAF9F6] p-2.5 border border-gray-100">
                  <span className="text-gray-400 block text-[10px]">체중</span>
          ...
          <div className="bg-white p-8 border border-dashed border-gray-300 text-center space-y-3">
            <p className="text-xs text-gray-500">아직 등록된 반려동물이 없습니다.</p>
            <button
              onClick={onOpenApplyModal}
              className="px-6 py-2.5 bg-[#144A42] text-white font-bold text-xs"
            >
              지금 동물등록 신청하기
            </button>
          </div>
      ...
      <div className="mb-10">
        <h2 className="text-lg font-bold text-[#144A42] mb-4 flex items-center gap-2">
          <ClockIcon className="w-5 h-5 text-[#144A42]" />
          <span>동물등록 신청 현황 및 배송 추적</span>
        </h2>

        <div className="space-y-4">
          {applications.map((app) => (
            <div key={app.id} className="bg-white p-6 border border-[#EAE3D6] shadow-xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-gray-100 gap-2">
                <div>
                  <span className="text-xs font-mono font-bold text-[#144A42]">{app.id}</span>
                  <h4 className="text-base font-bold text-gray-900 mt-0.5">
                    {app.petName} ({app.type})
                  </h4>
                  <span className="text-[11px] text-gray-400">신청일시: {app.appliedDate}</span>
                </div>
              ...
              {/* Shipping info */}
              <div className="py-3 text-xs flex justify-between items-center text-gray-600">
                <span>배송 정보: <strong className="text-gray-900">{app.trackingNumber}</strong></span>
                <span className="text-emerald-700 font-semibold">서류 심사 및 등록 완료</span>
              </div>

              {/* Timeline */}
              <div className="bg-[#FAF9F6] p-4 space-y-3 mt-2 border border-gray-100">
                <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">진행 타임라인</p>
                <div className="space-y-2 text-xs">
                  {app.history.map((h, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <span className="w-2 h-2 bg-[#144A42] mt-1.5 shrink-0"></span>
                      <div>
                        <span className="font-semibold text-gray-900">{h.title}</span>
                        <span className="text-gray-400 text-[11px] ml-2">({h.date})</span>
      ...
      <div>
        ...
        {bookmarkedItems.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {bookmarkedItems.map((item) => (
              <div 
                ...
                className="bg-white p-5 border border-gray-200 shadow-xs hover:shadow-md transition cursor-pointer flex justify-between items-center"
              >
                <div>
                  <span className="text-[10px] font-bold text-[#144A42] bg-[#EAF5F2] px-2 py-0.5">
                    {item.categoryName}
                  </span>
                  <h4 className="font-bold text-sm text-gray-900 mt-1">{item.name}</h4>
                  <p className="text-xs text-gray-500">{item.location}</p>
                  <p className="text-xs font-semibold text-[#144A42] mt-1">{item.benefit}</p>
                </div>
          ...
          <p className="text-xs text-gray-400 p-6 bg-white border border-gray-100 text-center">
